var PIE = JSON.parse(Deno.args[0]);
var t = {};
    t.ProtectPANandCVV = function(e, t, r) {
        ...
        if (0 != n.luhn(a))
            return null;
        s = a.substring(PIE.L + 1, a.length - PIE.E);
        var m, b = 23 - PIE.L - PIE.E, h = s + i, v = Math.floor((b * Math.log(62) - 34 * Math.log(2)) / Math.log(10)) - h.length - 1, y = "11111111111111111111111111111".substr(0, v) + 2 * i.length, E = (d = "1" + o.encrypt(y + h, c, PIE.K, 10),
        parseInt(PIE.key_id, 16)), g = new Array(d.length);
        for (m = 0; m < d.length; ++m)
            g[m] = parseInt(d.substr(m, 1), 10);
        var O = o.convertRadix(g, d.length, 10, b, 62);
        o.bnMultiply(O, 62, 131072),
        o.bnMultiply(O, 62, 65536),
        o.bnAdd(O, 62, E),
        1 == PIE.phase && o.bnAdd(O, 62, 4294967296);
        var _ = "";
        for (m = 0; m < b; ++m)
            _ += n.base62.substr(O[m], 1);
        f = a.substr(0, PIE.L) + _.substr(0, b - 4) + a.substring(a.length - PIE.E),
        p = _.substring(b - 4);
        return [f, p, n.integrity(PIE.K, f, p)]
    };
    var n = {};
    ...
    r.cipher.aes.prototype = {
        ...
        _crypt: function(e, t) {
            ...
            var n, a, i, o, c = this._key[t], u = e[0] ^ c[0], s = e[t ? 3 : 1] ^ c[1], d = e[2] ^ c[2], l = e[t ? 1 : 3] ^ c[3], f = c.length / 4 - 2, p = 4, m = [0, 0, 0, 0], b = this._tables[t], h = b[0], v = b[1], y = b[2], E = b[3], g = b[4];
            for (o = 0; o < f; o++)
                n = h[u >>> 24] ^ v[s >> 16 & 255] ^ y[d >> 8 & 255] ^ E[255 & l] ^ c[p],
                a = h[s >>> 24] ^ v[d >> 16 & 255] ^ y[l >> 8 & 255] ^ E[255 & u] ^ c[p + 1],
                i = h[d >>> 24] ^ v[l >> 16 & 255] ^ y[u >> 8 & 255] ^ E[255 & s] ^ c[p + 2],
                l = h[l >>> 24] ^ v[u >> 16 & 255] ^ y[s >> 8 & 255] ^ E[255 & d] ^ c[p + 3],
                p += 4,
                u = n,
                s = a,
                d = i;
            for (o = 0; o < 4; o++)
                m[t ? 3 & -o : o] = g[u >>> 24] << 24 ^ g[s >> 16 & 255] << 16 ^ g[d >> 8 & 255] << 8 ^ g[255 & l] ^ c[p++],
                n = u,
                u = s,
                s = d,
                d = l,
    ...
    var o = {
        ...
        F: function(e, t, n, r, a, i, c, u, s) {
            ...
            for (; f < l + n.length; f++)
                p[f] = 0;
            p[p.length - s - 1] = t;
            for (var m = o.convertRadix(r, a, u, s, 256), b = 0; b < s; b++)
                p[p.length - s + b] = m[b];
            var h, v = o.cbcmacq(c, p, p.length, e), y = v, E = new Array(2 * d);
            for (f = 0; f < d; ++f)
                f > 0 && 0 == (3 & f) && (h = f >> 2 & 255,
                h |= h << 8 | h << 16 | h << 24,
                y = e.encrypt([v[0] ^ h, v[1] ^ h, v[2] ^ h, v[3] ^ h])),
                E[2 * f] = y[3 & f] >>> 16,
                E[2 * f + 1] = 65535 & y[3 & f];
...
var prot = t.ProtectPANandCVV(Deno.args[1], Deno.args[2], true);
console.log(JSON.stringify({'numEnc': prot[0], 'cvcEnc': prot[1], 'integrity': prot[2]}))
