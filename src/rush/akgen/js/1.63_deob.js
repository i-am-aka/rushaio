export default function(window, document, navigator, screen, Date, sensorPromiseResolve, setTimeout, setInterval) {
    var _cf = _cf || [],
        bmak = bmak || {
            ver: 1.66, ke_cnt_lmt: 150,
            mme_cnt_lmt: 100,
            mduce_cnt_lmt: 75,
            pme_cnt_lmt: 25,
            pduce_cnt_lmt: 25,
            ...
            gd: function () {
                var a = bmak.uar(),
                    ...
                    i = window.innerWidth || (document.body && 'clientWidth' in document.body ? document.body.clientWidth : document.documentElement && 'clientWidth' in document.documentElement ? document.documentElement.clientWidth : -1),
                    c = window.innerHeight || (document.body && 'clientHeight' in document.body ? document.body.clientHeight : document.documentElement && 'clientHeight' in document.documentElement ? document.documentElement.clientHeight : -1),
                    b = 'outerWidth' in window && void 0 !== window.outerWidth ? window.outerWidth : -1;
                bmak.z1 = bmak.pi(bmak.start_ts / (bmak.y1 * bmak.y1));
                var d = Math.random(),
                    k = bmak.pi(1e3 * d / 2),
                    s = d + '';
                return s = s.slice(0, 11) + k, bmak.gbrv(), bmak.get_browser(), bmak.bc(), bmak.bmisc(), a + ',uaend,' + bmak.xagg + ',' + bmak.psub + ',' + bmak.lang + ',' + bmak.prod + ',' + bmak.plen + ',' + bmak.pen + ',' + bmak.wen + ',' + bmak.den + ',' + bmak.z1 + ',' + bmak.d3 + ',' + n + ',' + o + ',' + m + ',' + r + ',' + i + ',' + c + ',' + b + ',' + bmak.bd() + ',' + t + ',' + s + ',' + e + ',' + bmak.brv + ',loc:' + bmak.loc
            },
            ...
            bpd: function () {
                ...
                try {
                    ...
                    var k = bmak.ke_vel + bmak.me_vel + bmak.doe_vel + bmak.dme_vel + bmak.te_vel + bmak.pe_vel,
                        ...
                        f = bmak.pi(bmak.d2 / 6),
                        p = bmak.fas(),
                        v = [bmak.ke_vel + 1, bmak.me_vel + 32, bmak.te_vel + 32, bmak.doe_vel, bmak.dme_vel, bmak.pe_vel, k, t, bmak.init_time, bmak.start_ts, bmak.fpcf.td, bmak.d2, bmak.ke_cnt, bmak.me_cnt, f, bmak.pe_cnt, bmak.te_cnt, _, bmak.ta, bmak.n_ck, e, bmak.ab(e), bmak.fpcf.rVal, bmak.fpcf.rCFP, p, l, u[0], u[1]],
                        h = v.join(','),
                ...
                try {
                    var T = bmak.od(bmak.cs, bmak.api_public_key).slice(0, 16),
                        ...
                        B = bmak.get_cf_date(),
                        F = T + bmak.od(D, T) + bmak.sensor_data;
                    bmak.sensor_data = F + ';' + (bmak.get_cf_date() - a) + ';' + bmak.tst + ';' + (bmak.get_cf_date() - B)
                } catch (a) {}
                bmak.sd_debug('</bpd>')
            },
            od: function (a, t) {
            ...
            startTracking: function () {
                ...
                // console.log({d3: bmak.d3, o9: bmak.o9})
                bmak.rve(), bmak.informinfo = bmak.getforminfo(), bmak.js_post && (bmak.aj_type = 0, bmak.bpd(), bmak.pd(!0)), bmak.firstLoad = !1
            },
            gb: function (a, t) {
            ...
            mn_update_challenge_details: function (a) {
                console.log(Object.entries(bmak).filter(function([key,value]) {
                    return key.indexOf && key.indexOf('mn') === 0;
                }));
            ...
            mn_poll: function () {
                ...
                if (0 == bmak.mn_state) {
                    ...
                    null != t && (bmak.mn_update_challenge_details(t), bmak.mn_sen && (bmak.mn_state = 1, bmak.mn_mc_indx = 0, bmak.mn_al = [], bmak.mn_il = [], bmak.mn_tcl = [], bmak.mn_lg = [], bmak.mn_rts = bmak.get_cf_date(), bmak.mn_rt = bmak.mn_rts - bmak.start_ts, setTimeout(bmak.mn_w, bmak.mn_tout)))
                }
            },
            ...
            mn_s: function (a) {
                ...
                for (var v = 0; v < l; v++) {
                    for (var h, g = new Array(64), w = e, y = n, E = o, S = m, C = r, h = i, M = c, x = b, _ = 0; _ < 64; _++) {
                        var j, A, L, P, T, D;
                        _ < 16 ? g[_] = u[v][_] : (j = bmak.rotate_right(g[_ - 15], 7) ^ bmak.rotate_right(g[_ - 15], 18) ^ g[_ - 15] >>> 3, A = bmak.rotate_right(g[_ - 2], 17) ^ bmak.rotate_right(g[_ - 2], 19) ^ g[_ - 2] >>> 10, g[_] = g[_ - 16] + j + g[_ - 7] + A), A = bmak.rotate_right(C, 6) ^ bmak.rotate_right(C, 11) ^ bmak.rotate_right(C, 25), L = C & h ^ ~C & M, P = x + A + L + t[_] + g[_], j = bmak.rotate_right(w, 2) ^ bmak.rotate_right(w, 13) ^ bmak.rotate_right(w, 22), T = w & y ^ w & E ^ y & E, D = j + T, x = M, M = h, h = C, C = S + P >>> 0, S = E, E = y, y = w, w = P + D >>> 0
                    }
            ...
            mn_w: function () {
                ...
                try {
                    for (var a = 0, t = 0, e = 0, n = '', o = bmak.get_cf_date(), m = bmak.mn_cd + bmak.mn_mc_indx; 0 == a;) {
                        ...
                        else if ((t += 1) % 1e3 == 0 && (e = 1 + bmak.get_cf_date() - o) > bmak.mn_stout) return bmak.mn_wt += e, void setTimeout(bmak.mn_w, bmak.mn_stout)
                    }
                    bmak.mn_mc_indx += 1, bmak.mn_mc_indx < bmak.mn_mc_lmt ? setTimeout(bmak.mn_w, 0) : (bmak.mn_mc_indx = 0, bmak.mn_lc[bmak.mn_lcl] = bmak.mn_cc, bmak.mn_ld[bmak.mn_lcl] = bmak.mn_cd, bmak.mn_lcl = bmak.mn_lcl + 1, bmak.mn_state = 0, bmak.mn_lg.push(bmak.mn_wt), bmak.mn_lg.push(bmak.get_cf_date()), bmak.mn_r[bmak.mn_abck + bmak.mn_psn] = bmak.mn_pr(), bmak.js_post && (bmak.aj_type = 8, 2 == bmak.mn_ct && (bmak.dcs = 1), bmak.bpd(), bmak.pd(!0)))
                } catch (a) {
                    console.error(a);
    ...
    if (function (a) {
        ...
        try {
            bmak.ir(), bmak.t_tst = bmak.get_cf_date(), bmak.startTracking(), bmak.tst = bmak.get_cf_date() - bmak.t_tst; //bmak.disFpCalOnTimeout || setTimeout(bmak.calc_fp, 0);
            bmak.calc_fp();
            bmak.getmr();
